 ...
 * No necesita Python ni servidor — se ejecuta en el navegador.
 */
...
const CACHE_KEY    = 'cines-murcia-live-v2';
...
// Encuentra el h3 más cercano que precede a 'el' en orden de documento
...
async function batchMap(items, fn, size = 8) {
  const out = [];
...
function parseNeocinePage(html) {
  ...
  doc.querySelectorAll('a[href*="entradas.com"][href*="/evento/"]').forEach(link => {
    if (!/^\d{1,2}:\d{2}$/.test(link.textContent.trim())) return;
    ...
    // Clasificación — imagen calificacion en ancestros
    let rating = '?';
...
async function scrapeFilmotecaList(todayStr, maxDateStr) {
  ...
  for (let ofs = 0; ofs < 200; ofs += 12) {
    try {
      ...
      if (!page.length) { if (++emptyPages >= 3) break; continue; }

      let added = 0;
      page.forEach(e => { if (!seen.has(e.url)) { seen.add(e.url); all.push(e); added++; } });
      emptyPages = added ? 0 : (++emptyPages >= 3 ? Infinity : emptyPages);
      if (emptyPages === Infinity) break;
    } catch { break; }
  }
  return all;
}
  ...
  try {
    ...
    for (const img of doc.querySelectorAll('img')) {
      ...
      if (src.includes('integra.servlets.Imagenes')) {
        poster = src.startsWith('http') ? src : FILMOTECA_BASE + src;
        break;
      }
    }
    ...
    return { sala: 'Sala única', poster: null, overview: null };
  }
}
...
async function fetchTMDBMovie(title, genreMap) {
  for (const lang of ['es', 'en']) {
    try {
      const r = await fetch(
        `${TMDB_SEARCH}?api_key=${TMDB_KEY}&query=${encodeURIComponent(title)}&language=${lang}`
      );
      const d = await r.json();
      const m = (d.results || [])[0];
      if (m) return {
...
async function fetchLiveData(onProgress = () => {}) {
  ...
  // 2. GraphQL para fechas/horas/sala de neocine
  const allIds = [...myrteaShows, ...centrofamaShows].map(s => s.showId);
  onProgress(`Cargando horarios (${allIds.length} sesiones)…`, 18);
  const gql = await enrichWithGraphQL(allIds, onProgress); // progreso 18→55%

  // 3. Detalles Filmoteca (sala, carátula, sinopsis)
  ...
  filmoEvents.forEach((e, i) => {
    e.sala     = details[i].sala;
    e._poster  = details[i].poster;
    e._overview= details[i].overview;
  });
  ...
  for (const [cfg, shows] of [[CINEMAS_CFG[0], myrteaShows],[CINEMAS_CFG[1], centrofamaShows]]) {
    ...
    shows.forEach(show => {
      ...
      sessions.push({
        movie: show.movie, date, time: localTimeStr(dt),
  ...
  filmoEvents.forEach(s => {
    if (s._poster || s._overview) {
      movies[s.movie] = {
        poster: s._poster || null, overview: s._overview || null,
